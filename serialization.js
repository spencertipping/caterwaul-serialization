...
// Integers are little-endian encoded. This means that the least-significant base-94 digit comes first, not last. So the bytes [34, 33] means the number 1, not 94.
    ...
    radix_encode(n, l) = n /~![base <= n || xi <= l][(n / (base *= 94) >>> 0) % 94 /!char_encode] -seq -re- it.slice(1).join('') -where [base = 1 / 94],
    radix_decode(s)    = s.split('') /[0][x0 + char_decode(x) * (base *= 94)] -seq -where [base = 1 / 94],
